...
function shouldPromptTargetSelection(selector, candidates) {
    ...
    if (candidates.length > 1)
        return true;
    return (selector.type === "enemyFollower" ||
...
function promptSelectZoneCards(state, player, fromZone, count, action, matches, resumeActivate) {
    const next = structuredClone(state);
...
function promptSelectTarget(state, player, effect, candidates) {
    const next = structuredClone(state);
    next.pendingChoices = (0, effect_utils_1.withChoiceContext)(next, {
        type: "selectTarget",
        player,
...
function promptSearchDeckTop(state, player, top, filter, to, optional, playCostReduction, remainderTo = "cemetery", reveal) {
    const next = structuredClone(state);
    next.pendingChoices = (0, effect_utils_1.withChoiceContext)(next, {
        type: "searchDeckTop",
        player,
...
function promptSelectDeckSummon(state, player, top, filter, maxTotalCost, remainderTo) {
    const next = structuredClone(state);
    next.pendingChoices = (0, effect_utils_1.withChoiceContext)(next, {
...
function applyGrantKeyword(state, player, keyword, targets) {
    const next = structuredClone(state);
    const candidates = getTargetCandidates(next, player, targets);
    if (candidates.length === 0)
        return state;
    const found = (0, queries_1.findInstance)(next, candidates[0]);
    if (!found)
        return state;
...
function resolveEffect(state, effect, player, options) {
    ...
    switch (effect.op) {
        ...
        case "dealDamage": {
            ...
            else if (shouldPromptTargetSelection(effect.targets, candidates) &&
                !next.pendingChoices) {
        ...
        case "damageFollowerAndLeader": {
            ...
            else if (shouldPromptTargetSelection({ type: "enemyFollower", count: 1 }, candidates) &&
                !next.pendingChoices) {
                return promptSelectTarget(next, player, effect, candidates);
            }
            else {
        ...
        case "tutorFromDeck": {
            ...
            if (matches.length === 0)
                break;
            if (!next.pendingChoices) {
                return promptSelectZoneCard(next, player, "deck", effect.to, matches, undefined, effect.playCostReduction, effect.reveal);
            }
            break;
        }
        case "searchDeckChoose": {
            ...
            if (top.length === 0)
                break;
            if (!next.pendingChoices) {
                return promptSearchDeckTop(next, player, top, effect.filter, effect.to, effect.optional, effect.playCostReduction, effect.remainderTo ?? "cemetery", effect.reveal);
            }
            break;
        }
        case "engage": {
        ...
        case "reviveSelfFromCemetery": {
            ...
            if (idx < 0 || p.zones.field.length >= p.fieldLimit)
                break;
            const [card] = p.zones.cemetery.splice(idx, 1);
            p.zones.field.push(card);
            (0, confirmation_1.onFollowerEntersField)(next, card.instanceId, player);
        ...
        case "summonFromCemetery": {
            ...
            for (const card of [...matches]) {
                if (summoned >= toSummon || p.zones.field.length >= p.fieldLimit)
                    break;
                ...
                (0, confirmation_1.onFollowerEntersField)(next, picked.instanceId, player);
                summoned++;
            }
            break;
        }
        case "searchDeckSummonMultiple": {
            ...
            if (top.length === 0)
                break;
            if (!next.pendingChoices) {
                return promptSelectDeckSummon(next, player, top, effect.filter, effect.maxTotalCost, effect.remainderTo ?? "deckBottom");
            }
            break;
        }
